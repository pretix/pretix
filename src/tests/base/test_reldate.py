...
@pytest.mark.django_db
@scopes_disabled()
def test_relative_to_subevent(event):
    event.has_subevents = True
    event.save()
    se1 = event.subevents.create(
        name="Foobar",
        date_from=datetime(2023, 6, 27, 5, 0, 0, tzinfo=TOKYO),
        date_to=datetime(2023, 7, 27, 5, 0, 0, tzinfo=TOKYO),
        date_admission=datetime(2023, 6, 27, 6, 0, 0, tzinfo=TOKYO),
    )
    se2 = event.subevents.create(
        name="Foobar1",
        date_from=datetime(2024, 6, 27, 5, 0, 0, tzinfo=TOKYO),
        date_to=datetime(2024, 7, 27, 5, 0, 0, tzinfo=TOKYO),
        date_admission=datetime(2024, 6, 27, 6, 0, 0, tzinfo=TOKYO),
    )
    ...
    rdw = RelativeDate(days=1, time=None, base_date_name='order.subevents__date_from__first', minutes=None, is_after=True)
    assert rdw.datetime(order).astimezone(TOKYO) == datetime(2023, 6, 28, 5, 0, 0, tzinfo=TOKYO)
    assert rdw.to_string() == 'RELDATE/1/-/order.subevents__date_from__first/after'
    rdw = RelativeDate(days=1, time=None, base_date_name='order.subevents__date_from__last', minutes=None, is_after=True)
    assert rdw.datetime(order).astimezone(TOKYO) == datetime(2024, 6, 28, 5, 0, 0, tzinfo=TOKYO)
    assert rdw.to_string() == 'RELDATE/1/-/order.subevents__date_from__last/after'

    rdw = RelativeDate(days=1, time=None, base_date_name='order.subevents__date_to__first', minutes=None, is_after=True)
    assert rdw.datetime(order).astimezone(TOKYO) == datetime(2023, 7, 28, 5, 0, 0, tzinfo=TOKYO)
    assert rdw.to_string() == 'RELDATE/1/-/order.subevents__date_to__first/after'
    rdw = RelativeDate(days=1, time=None, base_date_name='order.subevents__date_to__last', minutes=None, is_after=True)
    assert rdw.datetime(order).astimezone(TOKYO) == datetime(2024, 7, 28, 5, 0, 0, tzinfo=TOKYO)
    assert rdw.to_string() == 'RELDATE/1/-/order.subevents__date_to__last/after'

    rdw = RelativeDate(days=1, time=None, base_date_name='order.subevents__date_admission__first', minutes=None, is_after=True)
    assert rdw.datetime(order).astimezone(TOKYO) == datetime(2023, 6, 28, 6, 0, 0, tzinfo=TOKYO)
    assert rdw.to_string() == 'RELDATE/1/-/order.subevents__date_admission__first/after'
    rdw = RelativeDate(days=1, time=None, base_date_name='order.subevents__date_admission__last', minutes=None, is_after=True)
    assert rdw.datetime(order).astimezone(TOKYO) == datetime(2024, 6, 28, 6, 0, 0, tzinfo=TOKYO)
    assert rdw.to_string() == 'RELDATE/1/-/order.subevents__date_admission__last/after'
