...
def _perform_order(event: Event, payment_provider: str, position_ids: List[str],
    ...
    if not invoice and invoice_qualified(order):
        if event.settings.get('invoice_generate') == 'True' or (event.settings.get('invoice_generate') == 'paid' and payment.payment_provider.requires_invoice_immediately):
            invoice = generate_invoice(
                order,
        ...
        if self.reissue_invoice and self._invoice_dirty:
            ...
            if invoice_qualified(self.order) and \
                (i or
                 ...
                 (self.open_payment is not None and self.event.settings.invoice_generate == 'paid' and self.open_payment.payment_provider.requires_invoice_immediately)):
