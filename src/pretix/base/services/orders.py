...
def _perform_order(event: Event, payment_provider: str, position_ids: List[str],
    ...
    invoice = order.invoices.last()  # Might be generated by plugin already
    if invoice_qualified(order) and not invoice and payment.payment_provider.create_invoice_immediately(order):
        invoice = generate_invoice(
            order,
        ...
        if self.reissue_invoice and self._invoice_dirty:
            ...
            if invoice_qualified(self.order) and \
                (i or
                 ...
                 self.open_payment is not None and self.open_payment.payment_provider.create_invoice_immediately(self.order)):
