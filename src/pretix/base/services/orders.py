            ...
            with language(order.locale, order.event.settings.region):
                ...
                try:
                    order.send_mail(
                        ...
                        invoices=invoices if transmit_invoice_mail else []
                    )
                except SendMailException:
...
class OrderChangeManager:
    ...
    def commit(self, check_quotas=True):
        ...
        transmit_invoice_mail = not transmit_invoice_task and self.event.settings.invoice_email_attachment and self.order.email

        if self.notify:
            notify_user_changed_order(
                self.order, self.user, self.auth,
                self._invoices if transmit_invoice_mail else []
            )
            if self.split_order:
                notify_user_changed_order(
                    self.split_order, self.user, self.auth,
                    list(self.split_order.invoices.all()) if transmit_invoice_mail else []
                )
