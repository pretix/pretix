...
This is a organizer-level plugin signal (not an event-level signal). Organizer-level and
hybrid-level plugins, will receive it if they're active for the current organizer.

**Deprecation Notice:** Currently, event-level plugin can always receive this signal, regardless
of activation. In the future, event-level plugins will not be allowed to register to 
organizer-level signals. 
