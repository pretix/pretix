...
fabric.Textcontainer = fabric.util.createClass(fabric.Rect, {
	...
	_internalTextbox: function () {
		...
		while (true) {
			let tmptext = new fabric.Textbox(text, {
				_wordJoiners: /[ \t\r\u200B]/u,
				left: 0,
				top: 0,
				originY: 'top',
			...
			tmptext.setCoords()
			let lineHeights = 0
			for (let i = 0, len = tmptext._textLines.length; i < len; i++) {
				let heightOfLine = tmptext.getHeightOfLine(i)
			...
			if (lineHeights > this.height) { // we can do larger steps for height
				fontSize = fontSize - Math.max(1.0, fontSize * 0.1)
			} else {
				fontSize = fontSize - Math.max(0.25, fontSize * 0.025)
			}
		}
	},
	...
	_render: function (ctx) {
        ...
        ctx.fill();
         */
		let { textbox, height, width } = this._internalTextbox()
...
var editor = {
	...
	dump: function (objs) {
		let d = !objs ? JSON.parse(JSON.stringify(editor._other_page_objects)) : []
	...
	_load_page: function (page_number, dump) {
		let previous_dump = editor._fabric_loaded ? editor.dump() : []

		// Fetch the required page
		editor.pdf.getPage(page_number).then(function (page) {
			let canvas = document.getElementById('pdf-canvas')

			let scale = editor.$cva.width() / page.getViewport({ scale: 1.0 }).width
	...
	_init_page_nav: function () {
		if (editor.pdf_page_count === 1) {
			...
			for (i = 1; i <= editor.pdf_page_count; i++) {
				...
				let $a = $('<a>').text(i).attr('href', '#').attr('data-page', i).appendTo($li)
	...
	_ready: function () {
		...
		let isChromeBased = !!window.chrome
	...
	_update_toolbox_values: function () {
		...
		if (o.type === 'textarea' || o.type === 'barcodearea' || o.type === 'textcontainer') {
			...
			let $migrate_to = $('#toolbox-content option[data-old-value=\'' + o.content + '\']')
	...
	_update_values_from_toolbox: function (e) {
		...
		if (o.type === 'barcodearea') {
			...
			let verticalAlign = $('#toolbox-verticalalign').find('.active').attr('data-action')
			if (verticalAlign) {
	...
	_add_textcontainer: function () {
		let rect = new fabric.Textcontainer(editor._get_text_sample('event_name'), {
			left: 100,
			top: 100,
			width: 300,
			height: 29,
	...
	_duplicate: function () {
		let prevClipboad = editor.clipboard
		if (editor._copy()) {
			editor._paste()
	...
	_on_keydown: function (e) {
		let step = editor._mm2px(e.shiftKey ? 10 : (e.altKey ? 0.1 : 1))
		let thing = editor.fabric.getActiveObject()
	...
	_selectAll: function () {
		let selection = new fabric.ActiveSelection(editor.fabric._objects, { canvas: editor.fabric })
		editor.fabric.setActiveObject(selection)
	},
	...
	_source_save: function () {
		try {
			let data = JSON.parse($('#source-textarea').val())
			let valid = validate30(data)
