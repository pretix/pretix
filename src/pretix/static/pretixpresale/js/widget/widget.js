...
let padNumber = function (number, size) {
	let s = String(number)
	while (s.length < (size || 2)) { s = '0' + s }
	return s
}

let getISOWeeks = function (y) {
	let d, isLeap
...
let api = {
	...
	_postFormJSON: function (endpoint, form, callback, err_callback) {
		let params = [].filter.call(form.elements, function (el) {
...
Vue.component('Availbox', {
	...
	computed: {
		...
		unavailability_reason_message: function () {
			let reason = this.item.current_unavailability_reason || this.variation?.current_unavailability_reason
			if (reason) {
		...
		waiting_list_url: function () {
			let u = this.$root.target_url + 'w/' + widget_id + '/waitinglist/?locale=' + lang + '&item=' + this.item.id
			if (this.item.has_variations) {
	...
	methods: {
		on_step: function (e) {
			...
			let step = parseFloat(t.getAttribute('data-step'))
			let controls = document.getElementById(t.getAttribute('data-controls'))
...
Vue.component('Pricebox', {
	...
	computed: {
		...
		suggested_price_nonlocalized: function () {
			let price = this.suggested_price
			if (price === null) {
				price = this.price
			}
			if (this.$root.display_net_prices) {
...
let shared_methods = {
	buy: function (event) {
		...
		if (this.$root.is_button && this.$root.items.length === 0) {
			...
			let url = this.$root.formAction + '&locale=' + lang + '&ajax=1'
	...
	voucher_open: function (voucher) {
		let redirect_url = this.$root.voucherFormTarget + '&voucher=' + encodeURIComponent(voucher)
		if (this.$root.useIframe) {
...
let shared_widget_data = function () {
	return {
		async_task_id: null,
...
Vue.component('PretixOverlay', {
	...
	watch: {
		...
		'$root.frame_shown': function (newValue) {
			if (newValue) {
				let btn = this.$el?.querySelector('.pretix-widget-frame-close button')
...
Vue.component('PretixWidgetEventListFilterField', {
	props: {
		field: Object
	},
	computed: {
		...
		currentValue: function () {
			let filterParams = new URLSearchParams(this.$root.filter)
...
Vue.component('PretixWidgetEventListFilterForm', {
	methods: {
		onSubmit: function (e) {
			e.preventDefault()
...
let shared_root_methods = {
	...
	trigger_load_callback: function () {
		this.$nextTick(function () {
			for (let i = 0; i < window.PretixWidget._loaded.length; i++) {
	...
	trigger_close_callback: function () {
		this.$nextTick(function () {
			for (let i = 0; i < window.PretixWidget._closed.length; i++) {
	...
	reload: function (opt = {}) {
		...
		api._getJSON(url, function (data, xhr) {
			if (typeof xhr.responseURL !== 'undefined') {
				let new_url = xhr.responseURL.substr(0, xhr.responseURL.indexOf('/widget/product_list?') + 1)
				let old_url = url.substr(0, url.indexOf('/widget/product_list?') + 1)
				if (new_url !== old_url) {
					if (root.subevent) {
	...
	startwaiting: function () {
		let redirect_url = this.$root.target_url + 'w/' + widget_id + '/waitinglist/?iframe=1&locale=' + lang
		if (this.$root.subevent) {
	...
	startseating: function () {
		let redirect_url = this.$root.target_url + 'w/' + widget_id
		if (this.$root.subevent) {
			redirect_url += '/' + this.$root.subevent
		}
	...
	create_cart: function (callback) {
		let url = this.$root.target_url + 'w/' + widget_id + '/cart/create?ajax=1'
...
let shared_root_computed = {
	...
	formTarget: function () {
		...
		let is_android = navigator.userAgent.toLowerCase().indexOf('android') > -1
	...
	voucherFormTarget: function () {
		let form_target = this.target_url + 'w/' + widget_id + '/redeem?iframe=1&locale=' + lang
		if (this.get_cart_id()) {
	...
	formAction: function () {
		...
		let checkout_url = '/' + this.target_url.replace(/^[^\/]+:\/\/([^\/]+)\//, '') + 'w/' + widget_id + '/'
		...
		let form_target = this.target_url + 'w/' + widget_id + '/cart/add?iframe=1&next=' + encodeURIComponent(checkout_url)
		if (this.get_cart_id()) {
			form_target += '&take_cart_id=' + encodeURIComponent(this.get_cart_id())
		}
		form_target += this.$root.consent_parameter
	...
	newTabTarget: function () {
		let target = this.target_url
		if (this.subevent) {
			target = this.target_url + this.subevent + '/'
		}
		let parameters = this.$root.consent_parameter
		if (this.$root.additionalURLParams) {
	...
	showPrices: function () {
		let has_priced = false
		let cnt_items = 0
		for (let i = 0; i < this.categories.length; i++) {
			for (let j = 0; j < this.categories[i].items.length; j++) {
				let item = this.categories[i].items[j]
	...
	widget_data_json: function () {
		let cloned_data = Object.assign({}, this.widget_data)
...
let create_overlay = function (app) {
	let elem = document.createElement('pretix-overlay')
	document.body.appendChild(elem)

	let framechild = new Vue({
		el: elem,
		props: {
			frame_src: String,
		},
		...
		watch: {
			...
			frame_loading: function (newValue) {
				let dialog = this.$el?.querySelector('dialog.pretix-widget-frame-holder')
				if (newValue) {
					if (!dialog.open) {
						dialog.showModal()
					}
...
let create_widget = function (element, html_id = null) {
	let target_url = element.attributes.event.value
	if (!target_url.match(/\/$/)) {
		target_url += '/'
	}
	let voucher = element.attributes.voucher ? element.attributes.voucher.value : null
	let subevent = element.attributes.subevent ? element.attributes.subevent.value : null
	let style = element.attributes['list-type'] ? element.attributes['list-type'].value : (element.attributes.style ? element.attributes.style.value : null)
	let skip_ssl = element.attributes['skip-ssl-check'] ? true : false
	let disable_iframe = element.attributes['disable-iframe'] ? true : false
	let disable_vouchers = element.attributes['disable-vouchers'] ? true : false
	let disable_filters = element.attributes['disable-filters'] ? true : false
	let display_event_info = element.getAttribute('display-event-info') // null means "auto" (as before), everything other than "false" is true
	if (display_event_info !== null && display_event_info !== 'auto') {
		display_event_info = display_event_info !== 'false'
	} else {
		display_event_info = null
	}
	let widget_data = JSON.parse(JSON.stringify(window.PretixWidget.widget_data))
	let filter = element.attributes.filter ? element.attributes.filter.value : null
	let items = element.attributes.items ? element.attributes.items.value : null
	let variations = element.attributes.variations ? element.attributes.variations.value : null
	let categories = element.attributes.categories ? element.attributes.categories.value : null
...
let create_button = function (element, html_id = null) {
	let target_url = element.attributes.event.value
	if (!target_url.match(/\/$/)) {
		target_url += '/'
	}
	let voucher = element.attributes.voucher ? element.attributes.voucher.value : null
	let subevent = element.attributes.subevent ? element.attributes.subevent.value : null
	let raw_items = element.attributes.items ? element.attributes.items.value : ''
	let skip_ssl = element.attributes['skip-ssl-check'] ? true : false
	let disable_iframe = element.attributes['disable-iframe'] ? true : false
	let keep_cart = element.attributes['keep-cart'] ? true : false
	let button_text = element.innerHTML
	let widget_data = JSON.parse(JSON.stringify(window.PretixWidget.widget_data))
	...
	let itemsplit = raw_items.split(',')
	let items = []
	for (var i = 0; i < itemsplit.length; i++) {
		if (itemsplit[i].indexOf('=') > 0) {
			let splitthis = itemsplit[i].split('=')
...
window.PretixWidget.buildWidgets = function () {
	...
	docReady(function () {
		let widgets = document.querySelectorAll('pretix-widget, div.pretix-widget-compat')
		let wlength = widgets.length
		for (var i = 0; i < wlength; i++) {
			let widget = widgets[i]
			widgetlist.push(create_widget(widget, widget.id || 'pretix-widget-' + i))
		}

		let buttons = document.querySelectorAll('pretix-button, div.pretix-button-compat')
		let blength = buttons.length
...
window.PretixWidget.open = function (target_url, voucher, subevent, items, widget_data, skip_ssl_check, disable_iframe) {
	...
	let all_widget_data = JSON.parse(JSON.stringify(window.PretixWidget.widget_data))
	if (widget_data) {
