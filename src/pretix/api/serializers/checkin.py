...
class CheckinRPCRedeemInputSerializer(serializers.Serializer):
    ...
    canceled_supported = serializers.BooleanField(default=False, required=False)
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['lists'].child_relation.queryset = CheckinList.objects.filter(event__in=self.context['events'])
