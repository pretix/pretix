"""
UI Test: Create Event and Verify It Appears in Events List

Issue:               630:P1 - UI test for create event and verify it appears in events list
Test Layer:          UI Test
Priority:            Medium
Behavior Protected:  Prevents regressions where creating an event does not
                     reflect immediately in the events list.

Acceptance Criteria:
 - Test navigates to the event creation form
 - Test fills in required event fields and submits
 - Test verifies the new event appears in the events list by name

Test Approach:
 - Use Playwright to interact with the event creation UI
 - Assert the event name is visible in the list after creation
...
# The implementation below is the reference spec (test-create-event.spec.ts).
# ---------------------------------------------------------------------------
