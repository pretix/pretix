...
You can access the consent state through the ``window.__pretix_cookie_consent`` variable. Whenever the
value changes, a ``pretix:cookie-consent-updated`` event is fired on the ``document`` object.
...
===================================================== =====================================================
``__pretix_cookie_consent === undefined``             Your JavaScript has loaded before the cookie consent
                                                      script. Wait for the event to be fired, then try again,
                                                      do not yet set a cookie.
``__pretix_cookie_consent === null``                  The cookie consent mechanism has not been enabled. This
                                                      usually means that you can set cookies however you like.
``__pretix_cookie_consent[identifier] === undefined`` The cookie consent mechanism is loaded, but has no data
                                                      on your cookie yet, wait for the event to be fired, do not
                                                      yet set a cookie.
``__pretix_cookie_consent[identifier] === true``      The user has consented to your cookie.
``__pretix_cookie_consent[identifier] === false``     The user has actively rejected your cookie.
===================================================== =====================================================
...
.. code-block:: javascript
     ...
     if (window.__pretix_cookie_consent !== null && !(window.__pretix_cookie_consent || {}).facebook) {
         fbq('consent', 'revoke');
     }
     fbq('init', ...);
     document.addEventListener('pretix:cookie-consent-updated', function (e) {
         fbq('consent', window.__pretix_cookie_consent.facebook ? 'grant' : 'revoke')
     })
...
.. code-block:: javascript
     ...
     if (typeof window.__pretix_cookie_consent === "undefined") {
         // Cookie consent tool has not loaded yet, wait for it to load, then try again
         document.addEventListener('pretix:cookie-consent-updated', function () {
             execute_function()
         })
     } else if (window.__pretix_cookie_consent === null) {
         // Cookie consent tool is disabled
         execute_function()
     } else {
         // Cookie tool is available
         if (window.__pretix_cookie_consent[identifier] === true) {
             execute_function()
         } else {
             document.addEventListener('pretix:cookie-consent-updated', function () {
                 if (window.__pretix_cookie_consent[identifier] === true) {
