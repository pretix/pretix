...
api_meta_info                         object                     Meta-data for that order. Only available through API.
...
.. http:patch:: /api/v1/organizers/(organizer)/events/(event)/orders/(code)/
   ...
   * ``api_meta_info`` (you always need to supply the full object, or ``null`` to delete the current value)
